...
8. **Facility slot is free** — a `:schedule-test-session` must not place the verified target session into a facility slot already occupied at the same `scheduled-start` by ANOTHER registered session. The per-session checks (1/2/3) look only at the target, so without this check a second session could be seated into an already-booked facility at the same start and both would auto-commit at Phase 3. A colliding schedule is rejected outright, never held and never auto-committed at Phase 3.
...
Supply request proposal shape (`:supplies` carries item strings from the
closed non-content consumable set; the governor rejects missing or
unrecognized items and any content-bearing payload):
